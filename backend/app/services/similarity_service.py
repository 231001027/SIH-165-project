"""Similar-report retrieval (blueprint Part 1.3 Stage 12). Computed on demand
over the stored fingerprint embeddings rather than persisted as a table, so
results are always fresh as new reports are analyzed."""
...
def find_similar_reports(db: Session, report_id: int, top_k: int = 5) -> list[dict]:
    ...
    target_vec = np.array(target_fp.embedding)
    corpus_vecs = np.array([fp.embedding for fp in all_fps])
    ...
    norms[norms == 0] = 1.0
    norm_corpus = corpus_vecs / norms
    similarities = norm_corpus @ norm_target

    order = np.argsort(similarities)[::-1][:top_k]
    results = []
    report_ids = [all_fps[i].report_id for i in order]
    reports = {r.id: r for r in db.query(Report).filter(Report.id.in_(report_ids)).all()}
    for i in order:
        fp = all_fps[i]
        r = reports.get(fp.report_id)
        if not r:
            continue
        ...
        results.append({
            ...
            # A PUBLIC_CORPUS match is a real, cited historical incident used only to
            # ground retrieval -- never an OIL/synthetic report -- so the frontend
            # renders it as an external citation, not an internal report link.
